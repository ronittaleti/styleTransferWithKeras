...
cImPath = 'data/base_images/dog.jpg'
sImPath = 'data/base_images/starrynight.jpg'
genImOutputPath = 'results/output'
...
def get_feature_reps(x, layer_names, model, sessionin):
    """
    ...
    for ln in layer_names:
        ...
        featRawShape = K.shape(featRaw).eval(session=sessionin)
        N_l = featRawShape[-1]
...
def get_total_loss(gImPlaceholder, alpha=1.0, beta=10000.0):
    F = get_feature_reps(gImPlaceholder, layer_names=[cLayerName], model=gModel, sessionin=sess)[0]
    Gs = get_feature_reps(gImPlaceholder, layer_names=sLayerNames, model=gModel, sessionin=sess)
...
def save_original_size(x, iteration, final, target_size=cImageSizeOrig):
    xIm = Image.fromarray(x)
    xIm = xIm.resize(target_size)
    if not final:
        xIm.save("results/output" + str(iteration) + ".jpg")
    else:
        xIm.save("results/outputFinal.jpg")

    return xIm


def iterateImage(eval, niter, x):
    i = 1
    while i <= niter:
        xopt, f_val, info = fmin_l_bfgs_b(eval, x, maxiter=1, fprime=get_grad, disp=True)
        xOut = postprocess_array(xopt)
        if i % 10 == 0:
            save_original_size(xOut, iteration=i, final=False)
            print('Image saved')
        i = i + 1

    save_original_size(xOut, iteration=0, final=True)
    return xopt
...
P = get_feature_reps(x=cImArr, layer_names=[cLayerName], model=cModel, sessionin=sess)[0]
As = get_feature_reps(x=sImArr, layer_names=sLayerNames, model=sModel, sessionin=sess)
...
xopt = iterateImage(calculate_loss, iterations, x_val)
